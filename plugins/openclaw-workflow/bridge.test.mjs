...
test('registered tool uses the stable workflow CLI with the active workspace', async () => {
  ...
  plugin.register({
    pluginConfig: {
      toolCommand: [process.env.DYNAMIC_WORKFLOW_TOOL || 'dynamic-workflow-tool'],
    },
    registerTool(factory, options) { registrations.push({factory, options}); },
  });
  try {
    const tools = registrations.flatMap(({factory}) =>
      typeof factory === 'function' ? factory({workspaceDir: workspace}) : factory);
    assert.deepEqual(
